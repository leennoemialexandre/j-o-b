...
const EMPTY_FORM = {
  ...
  resumeText:"", dateAdded: new Date().toISOString().split("T")[0],
};
...
function GoalBar({ goal, apps }) {
  ...
  if (goal.type==="weekly") {
    ...
    count=apps.filter(a=>{const d=new Date(a.dateAdded);return d>=ws&&d<we&&a.status!=="Saved";}).length;
    ...
    if (!goal.days.includes(todayIdx)) {
      ...
      count=apps.filter(a=>a.dateAdded===todayStr&&a.status!=="Saved").length;
...
export default function JobTracker() {
  ...
  useEffect(() => {
    setApps(loadLocal("jt_apps", []));
    setStages(loadLocal("jt_stages", DEFAULT_STAGES));
    const g = loadLocal("jt_goal", DEFAULT_GOAL);
    setGoal(g); setGD(g);
    setLoaded(true);
  }, []);

  const persistApps   = n => { setApps(n);   saveLocal("jt_apps", n); };
  const persistStages = n => { setStages(n); saveLocal("jt_stages", n); };
  const persistGoal   = n => { setGoal(n);   saveLocal("jt_goal", n); };
  ...
  const openDetail = a  => { setDA(a); setViewRes(false); setModal("detail"); };
  ...
  const submitForm = () => {
    ...
    const entry = {...form,industry,jobType,id:editId||Date.now()};
    delete entry.industryCustom; delete entry.jobTypeCustom;
    if(editId) persistApps(apps.map(a=>a.id===editId?entry:a));
    else persistApps([entry,...apps]);
    setModal(null); toast$(editId?"Application updated!":"Application added!");
  };

  const deleteApp    = id => { persistApps(apps.filter(a=>a.id!==id)); setModal(null); toast$("Deleted."); };
  const updateStatus = (id,status) => persistApps(apps.map(a=>a.id===id?{...a,status}:a));
  ...
  const S = {
    ...
    mb:       { background:"#fff", border:"1px solid #e2e8f0", borderRadius:"12px", padding:"28px", width:"100%", maxWidth:"520px", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.12)" },
  ...
  if (!loaded) return (
    <>
      <Head>
        ...
        <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Mono:wght@300;400&display=swap"/>
      </Head>
      ...
      <div style={S.app}>
        ...
        <div style={S.body}>
          {view==="board" ? (
                  ...
                  <div key={s} style={S.col}>
                    ...
                    {cards.map(app=>(
                      <div key={app.id} style={{...S.card,borderLeft:`3px solid ${c.accent}`}}
                        ...
                        <div style={S.cardM}>
                          {app.jobType&&<span style={{...S.tag,color:"#2563eb",borderColor:"#bfdbfe",background:"#eff6ff"}}>{app.jobType}</span>}
                          {app.cycle&&<span style={{...S.tag,color:"#94a3b8",borderColor:"#e2e8f0",background:"#f8fafc"}}>{app.cycle}</span>}
                          {app.resumeText&&<span style={{...S.tag,color:"#059669",borderColor:"#a7f3d0",background:"#ecfdf5"}}>resume</span>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <table style={S.table}>
              <thead><tr>{["Company","Role","Status","Type","Industry","Cycle","Date","Resume",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                ...
                {filtered.map(app=>{
                  ...
                  return (
                    <tr key={app.id} style={{cursor:"pointer"}} onClick={()=>openDetail(app)}
                      ...
                      <td style={{...S.td,color:"#94a3b8"}}>{app.jobType||"—"}</td>
                      <td style={{...S.td,color:"#94a3b8"}}>{app.industry||"—"}</td>
                      <td style={{...S.td,color:"#94a3b8"}}>{app.cycle||"—"}</td>
                      <td style={{...S.td,color:"#94a3b8"}}>{app.dateAdded||"—"}</td>
                      <td style={S.td}>{app.resumeText?<span style={{...S.tag,color:"#059669",borderColor:"#a7f3d0",background:"#ecfdf5",fontSize:"0.65rem"}}>✓</span>:<span style={{color:"#e2e8f0"}}>—</span>}</td>
        ...
        {(modal==="add"||modal==="edit")&&(
          <div style={S.ov} onClick={()=>setModal(null)}>
            <div style={S.mb} onClick={e=>e.stopPropagation()}>
              ...
              <div style={S.frow}>
                <label style={S.lbl}>Notes <span style={{color:"#cbd5e1",textTransform:"none",letterSpacing:0}}>(password hints, contacts…)</span></label>
                <textarea style={{...S.inp,minHeight:"60px",resize:"vertical"}} value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Any notes..."/>
              </div>
              <div style={S.frow}>
                <label style={S.lbl}>Resume Used</label>
              ...
              <div style={S.mfoot}>
                ...
                <button style={{...S.btn,...S.btnP}} onClick={submitForm}><Icon name="check" size={13}/> {modal==="edit"?"Save Changes":"Add Application"}</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div style={S.ov} onClick={()=>setModal(null)}>
              <div style={{...S.mb,maxWidth:"500px"}} onClick={e=>e.stopPropagation()}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px 20px",fontSize:"0.75rem",marginBottom:"16px"}}>
                  {[["Job Type",app.jobType],["Industry",app.industry],["Cycle",app.cycle],["Date Added",app.dateAdded]].map(([k,v])=>v&&(
                ...
                {app.resumeText&&<div style={{marginBottom:"16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                    <label style={S.lbl}>Resume</label>
                    <button style={{...S.btn,...S.btnG,padding:"4px 10px",fontSize:"0.65rem"}} onClick={()=>setViewRes(v=>!v)}>
                      <Icon name="eye" size={11}/>{viewRes?"Hide":"View"}
                    </button>
                  </div>
                  {viewRes
                    ? <div style={{color:"#475569",fontSize:"0.7rem",background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:"6px",padding:"12px",whiteSpace:"pre-wrap",maxHeight:"220px",overflowY:"auto",lineHeight:1.6}}>{app.resumeText}</div>
                    : <div style={{fontSize:"0.7rem",color:"#059669"}}>✓ Resume saved ({app.resumeText.length.toLocaleString()} chars)</div>
                  }
                </div>}
                <div style={S.mfoot}>
